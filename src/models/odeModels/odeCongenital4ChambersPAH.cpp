...
odeCongenital4ChambersPAH::odeCongenital4ChambersPAH(int modelVersion,int targetConfigMode){
  this->modelVersion = modelVersion;
  this->targetConfigMode = targetConfigMode;
}

odeCongenital4ChambersPAH::~odeCongenital4ChambersPAH(){
  
}

int odeCongenital4ChambersPAH::getParameterTotal(){
  return 45;
}

int odeCongenital4ChambersPAH::getHRIndex(){
  return 11;
}

string odeCongenital4ChambersPAH::getParamName(int index){
  string returnString;
  switch(index){
    case 0:
...
int odeCongenital4ChambersPAH::getStateTotal(){
  return 11;
}

int odeCongenital4ChambersPAH::getAuxStateTotal(){
  return 16;
}

void odeCongenital4ChambersPAH::getDefaultParameterLimits(stdVec& limits){
...
void odeCongenital4ChambersPAH::getDefaultParams(stdVec& params){
...
void odeCongenital4ChambersPAH::evalDeriv(double t,const stdVec& Xk,const stdVec& params,const stdMat& fn, stdVec& DXk, stdVec& auxOut, stdVec& Ind){
...
void odeCongenital4ChambersPAH::postProcess(double timeStep, int totalStepsOnSingleCycle, int totalSteps, const stdVec& params,const stdMat& outVals,const stdMat& auxOutVals, stdVec& results){
  ...
  // RIGHT VENTRICULAR VOLUME AT BEGINNING OF SYSTOLE (RVEDV)
  double minRV_EndVolume = outVals[2][startLastCycle];
  // RIGHT VENTRICULAR VOLUME AT THE END OF SYSTOLE (RVESV)
  double maxRV_EndVolume = outVals[2][stopLastCycle];
  ...
  // LEFT VENTRICLE END VOLUMES
  // LEFT VENTRICULAR VOLUME AT BEGINNING OF SYSTOLE (LVEDV)
  double minLV_EndVolume = outVals[3][startLastCycle];
  // LEFT VENTRICULAR VOLUME AT THE END OF SYSTOLE (LVESV)
  double maxLV_EndVolume = outVals[3][stopLastCycle];
  ...
  if((modelVersion >= 0001)&&(modelVersion < 0002)){
    ...
    results.push_back(minRV_EndVolume); // ip_0002_RVEDV_mri
    results.push_back(maxRV_EndVolume); // ip_0002_RVESV_mri
    results.push_back(PA_flow); // ip_002_QPA_mri
    results.push_back(minLV_EndVolume); // ip_0002_LVEDV_mri
    results.push_back(maxLV_EndVolume); // ip_0002_LVESV_mri
    results.push_back(A0_flow); // ip_002_QAO_mri
  }
}

void odeCongenital4ChambersPAH::getResultKeys(stdStringVec& keys){
  ...
  if((modelVersion >= 0001)&&(modelVersion < 0002)){
    ...
    if(targetConfigMode == ipTargetConfig_ExcludePAP){
      ...
      keys.push_back(string("RVEDV_mri"));
      keys.push_back(string("RVESV_mri"));
      keys.push_back(string("QPA_mri"));
      keys.push_back(string("LVEDV_mri"));
      keys.push_back(string("LVESV_mri"));
      keys.push_back(string("QAO_mri"));
...
void odeCongenital4ChambersPAH::getFinalOutputs(const stdVec& outputs,stdVec& outs){
  ...
  if((modelVersion >= 0001)&&(modelVersion < 0002)){
    ...
    if(targetConfigMode == ipTargetConfig_ExcludePAP){
      ...
      outs.push_back(outputs[ip_0002_RVEDV_mri]);
      outs.push_back(outputs[ip_0002_RVESV_mri]);
      outs.push_back(outputs[ip_0002_QPA_mri]);
      outs.push_back(outputs[ip_0002_LVEDV_mri]);
      outs.push_back(outputs[ip_0002_LVESV_mri]);
      outs.push_back(outputs[ip_0002_QAO_mri]);
...
void odeCongenital4ChambersPAH::getDataSTD(stdVec& stds){
...
void odeCongenital4ChambersPAH::getResultWeigths(stdVec& weights){
