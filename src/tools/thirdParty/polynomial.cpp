...
//    dimension D, for degree NM = 0, 1, 2, ...
//
//    For example, if D = 3, the ranking begins:
//
//    Rank  Sum    1  2  3
//    ----  ---   -- -- --
//       1    0    0  0  0
...
//
  ...
  for ( i = 1; i <= ks; i++ )
  {
    ...
    if ( tim1 + 1 <= xs[i-1] - 1 )
    {
      ...
      {
        rank = rank + i4_choose_poly ( ns - j, ks - i );
      }
    }
  }
  ...
  for ( n = 0; n < nm; n++ )
  {
    rank = rank + i4_choose_poly ( n + d - 1, n );
  }
...
//
  ...
  nksub = i4_choose_poly ( ns, ks );

  j = 1;

  for ( i = 1; i <= ks; i++ )
  {
    r = i4_choose_poly ( ns - j, ks - i );
...
void polynomial_dif ( int d, int o1, double c1[], int e1[], int dif[], 
...
//    polynomial 1.
//
//    Input, int DIF[D], indicates the number of 
...
//
{
  ...
  for ( j = 0; j < o1; j++ )
  {
    f1 = mono_unrank_grlex ( d, e1[j] );
    for ( i = 0; i < d; i++ )
    {
      c2[j] = c2[j] * i4_fall ( f1[i], dif[i] );
      f1[i] = i4_max_poly ( f1[i] - dif[i], 0 );
    }
    e2[j] = mono_rank_grlex ( d, f1 );
    delete [] f1;
  }
...
void polynomial_mul ( int d, int o1, double c1[], int e1[], int o2, double c2[],
  int e2[], int &o, double c[], int e[] )
...
//
{
  ...
  for ( j = 0; j < o2; j++ )
  {
    ...
    {
      c[o] = c1[i] * c2[j];
      f1 = mono_unrank_grlex ( d, e1[i] );
      f2 = mono_unrank_grlex ( d, e2[j] );
      for ( k = 0; k < d; k++ )
      {
        f[k] = f1[k] + f2[k];
      }
      e[o] = mono_rank_grlex ( d, f );
      delete [] f1;
      delete [] f2;
      o = o + 1;
    }
...
void polynomial_scale ( double s, int d, int o, double c[], int e[] )
